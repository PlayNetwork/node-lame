...
binding.mpg123_init();
process.once('exit', binding.mpg123_safe_exit);
...
function readDecodedData (decoder, done) {
	...
	return binding.mpg123_read(
		...
		(readResult, bytesRead, meta) => {
			...
			// check to see if ID3 data should be read
			if (decoder.id3) {
...
function Decoder (options) {
	...
	// internal property defining whether or not to read id3 tags
	this.id3 = (options && options.id3) ? options.id3 : false;
	...
	binding.mpg123_set_volume(this.handle, (options && 'volume' in options) ? options.volume : 1.0);
...
Decoder.prototype.close = function (callback) {
	debug('close(): called');

	// clean up the decoder
	return binding.mpg123_safe_exit();
};
